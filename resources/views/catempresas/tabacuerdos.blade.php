...
<div class="box box-primary">
    ...
    <div class="box-body">
      ...
      @foreach($catempresas->acuerdos as $key=>$acuerdos)
        <tr>
          <td>{{$key+1}}</td>
          <td>{{$acuerdos->fechasolicitud->format('d-M-Y')}}</td>
          <td>{{$acuerdos->nomsocio}}</td>
          <td>{{$acuerdos->cliente->nombre}} </a></td>
          <td title="{!! ($acuerdos->autorizado == 1) ? 'AUTORIZADO' : 'PENDIENTE' !!}">{!! ($acuerdos->autorizado) ? '<span class="badge bg-blue"><i class="fa fa-check"></i></span>' : '<span class="badge bg-red"><i class="fa fa-close"></i></span>' !!}</td>
